...
import { type User as UserSchemaType } from '../../db/schema';
 ...
 */
const findUser = async (token: string, jwtSecret: string, dbClient: DrizzleClient): Promise<UserSchemaType | undefined> => {
  const userClass = new User(dbClient);
...
const extractTokenAndGetUser = async (req: Request, jwtSecret: string, dbClient: DrizzleClient): Promise<UserSchemaType | undefined | null> => {
  const token = extractToken(req);
  if (!token) return null;
  const user = await findUser(token, jwtSecret, dbClient);
  return user;
};
